...
def update_graph(selected_data_type):
    ...
    for start, end, x in zip(edges[:-1], edges[1:], bin_centers):
        # Start of the bar
        fig.add_annotation(
            x=start,
            y=0,  # At the baseline
            yshift=-20,  # Shift downwards
            text=f"{start}",
            showarrow=False,
    ...
    # Update layout with titles and other properties
    fig.update_layout(
        title=f'{selected_data_type} Distribution',
        xaxis=dict(title=selected_data_type, showticklabels=False),  # Hide x-axis tick labels
