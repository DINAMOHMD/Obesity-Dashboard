...
layout = html.Div([
    # Title section
    html.Div([
        html.H1("Features Impact on Obesity", style={'textAlign': 'center', 'marginBottom': '20px'})
    ]),
    
    # First part: Dropdown to select a feature and corresponding count plot
    html.Div([
        html.Div([
            html.H4("NObeyesdad vs Categorical Features", style={'marginLeft': '20px'}),  # Left side title
            html.Div("Select a Feature", style={'textAlign': 'center', 'marginBottom': '10px'}),
            dcc.Dropdown(
                id='feature-dropdown',
                options=[{'label': 'Gender', 'value': 'Gender'},
                         {'label': 'Family History with Overweight', 'value': 'family_history_with_overweight'},
                         {'label': 'Frequent Consumption of High Caloric Food', 'value': 'FAVC'},
                         {'label': 'Consumption of Food Between Meals', 'value': 'CAEC'},
                         {'label': 'Smoking', 'value': 'SMOKE'},
                         {'label': 'Calorie Monitoring', 'value': 'SCC'},
                         {'label': 'Consumption of Alcohol', 'value': 'CALC'}],
                value='Gender',
    ...
    html.Div([
        html.Div([
            html.H4("NObeyesdad vs Mobility Type", style={'marginLeft': '20px'}),  # Left side title
            html.Div("Select a Mobility Type", style={'textAlign': 'center', 'marginBottom': '10px'}),
            dcc.Dropdown(
...
def update_countplot(selected_feature):
    if selected_feature:
        ...
        layout = go.Layout(title=f'Obesity Types by {selected_feature}',
